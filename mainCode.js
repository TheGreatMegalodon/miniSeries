...
var gameFunctions = {
  ...
  Abilities: {
    ...
    swap: {
      ...
      action: function(ship) {
        ...
        ship.set({
          type: shipRef[Math.floor(Math.random() * shipRef.length)],
          shield: 999,
          generator: 999,
          stats: 66666666,
          crystals: 180
  ...
  spawned: function(ship) {
    ...
    if (game.custom.roundStarted.state === true) {
      if (gameOptions.roundInfo[game.custom.roundStarted.round].abilityOdy) {
        ship.setUIComponent(gameFunctions.Abilities.swap.gameUIs.def);
      }
      gameFunctions.alert(
        ship, 
        `Corrent round ${game.custom.roundStarted.round}`, 
        `Good luck killing ${game.custom.mainFly}`, 
        "rgba(255,55,55,0.8)", 
        5000, 
        warning = {v1: 6, v2: 3, h: -3}
      );
      return;
    }
    gameFunctions.alert(
...
this.event = function(event, game) {
  switch (event.name) {
    ...
    case "ship_disconnected":
      if (gameInfo.players.fly.includes(event.ship)) gameInfo.players.fly.splice(gameInfo.players.fly.indexOf(event.ship), 1);
      if (gameInfo.players.ody.includes(event.ship)) gameInfo.players.ody.splice(gameInfo.players.ody.indexOf(event.ship), 1);
      break;
  }
};
